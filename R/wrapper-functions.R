...
#' Easy Identification of Differential Genes Across Cell Types and Conditions
#'
#' A wrapper function that performs differential gene analysis across all cell types
#' and conditions in single-cell data. Supports parallel processing and includes
#' gene annotation.
#'
#' @param object Seurat or SingleCellExperiment object with annotated cell types
...
#' @param annotate_pathways Whether to perform automatic pathway annotation
#' @param pathway_db Pathway database to use if annotate_pathways is TRUE
#' @param organism Organism code for pathway annotation
  ...
  if (annotate_pathways) {
    ...
    if (!exists("gene_sets")) {
      ...
      pathway_results <- runPathwayAnalysis(
        ...
        gene_sets = gene_sets,
        p_adj_cutoff = p_val_adj_threshold
      )
...
#' @param reference_group Optional, specify which group should be used as reference (ident.2 in FindMarkers)
#'
  ...
  if (return_plot && requireNamespace("ggplot2", quietly = TRUE)) {
    ...
    for (ct in names(all_results)) {
      ...
      if (nrow(plot_data) > 0) {
        ...
        top_up <- plot_data$gene[plot_data$significant & plot_data$avg_log2FC > 0]
        top_up <- top_up[order(plot_data$p_val_adj[plot_data$significant & plot_data$avg_log2FC > 0])]
        top_up <- head(top_up, 10)

        top_down <- plot_data$gene[plot_data$significant & plot_data$avg_log2FC < 0]
        top_down <- top_down[order(plot_data$p_val_adj[plot_data$significant & plot_data$avg_log2FC < 0])]
      ...
      p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = avg_log2FC, y = -log10(p_val_adj),
        ...
        ggplot2::geom_point(ggplot2::aes(alpha = significant)) +
        ggplot2::geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
        ggplot2::geom_hline(yintercept = -log10(p_val_adj_threshold), linetype = "dashed", color = "gray50") +
        ...
        ggplot2::labs(
          ...
          subtitle = paste0(sum(plot_data$significant), " significant DEGs (",
                            sum(plot_data$significant & plot_data$direction == "up"), " up, ",
                            sum(plot_data$significant & plot_data$direction == "down"), " down)"),
          x = "Log2 Fold Change",
      ...
      if (sum(plot_data$significant) > 0) {
