...
visualizeDEGHeatmap <- function(deg_results,
  ...
  if (cluster_rows) {
    row_dist <- stats::dist(mat_data, method = distance_method)
    ...
    p <- ggplot2::ggplot() +
      ...
      ggplot2::scale_fill_gradientn(
        colors = scales::col_numeric(
          ...
          domain = c(0, max(plot_data$count))
        )(seq(0, max(plot_data$count), length.out = 100)),
        name = legend_title
      ) +
...
#' Compare Gene Regulation Patterns Across Conditions
#'
#' Creates a multi-panel visualization that combines different plot types to provide
#' a comprehensive view of gene regulation patterns across cell types and conditions.
#'
#' @param deg_results Output from findDifferentialGenes or compareDEGs function
#' @param plot_types Vector of plot types to include: "heatmap", "barplot", "dotplot" (default = c("heatmap", "barplot"))
#' @param ncol Number of columns for the multi-panel layout (default = 1)
#' @param comparison_col Column name for the comparison categories
#' @param cell_type_col Column name for the cell types
#' @param up_color Color for up-regulated genes (default = "darkorange")
#' @param down_color Color for down-regulated genes (default = "deepskyblue")
#' @param title Overall title for the combined plot (default = "DEG Patterns Across Cell Types and Conditions")
#' @param filter_min_degs Minimum number of DEGs required to include a cell type-comparison (default = 0)
#' @param cluster_rows Logical. Whether to cluster rows in heatmap (default = TRUE)
#'
...
visualizeDEGPatterns <- function(deg_results,
  ...
  # Filter by minimum DEGs if requested
  if (filter_min_degs > 0) {
    summary_data <- summary_data %>%
      dplyr::filter(n_up + n_down >= filter_min_degs)
  }
  ...
  for (plot_type in plot_types) {
    if (plot_type == "heatmap") {
      ...
      # Create long format data for dotplot
      plot_data_long <- summary_data %>%
        tidyr::pivot_longer(
          cols = c("n_up", "n_down"),
          names_to = "regulation",
