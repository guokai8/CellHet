...
analyzeDEGTrajectory <- function(sce,
  ...
  gene_variability <- apply(bin_expression, 1, stats::var)
  ...
  if (smooth_method != "none") {
    ...
    if (smooth_method == "loess") {
      for (i in 1:length(top_genes)) {
        gene <- top_genes[i]
        loess_fit <- stats::loess(bin_expression[gene, ] ~ trajectory_midpoints)
      ...
      if (!requireNamespace("mgcv", quietly = TRUE)) {
        ...
        for (i in 1:length(top_genes)) {
          gene <- top_genes[i]
          gam_fit <- mgcv::gam(bin_expression[gene, ] ~ s(trajectory_midpoints))
          smoothed_expression[i, ] <- stats::predict(gam_fit, data.frame(trajectory_midpoints = trajectory_points))
        }
      }
    }
  } else {
  ...
  if (cluster_genes) {
    ...
    scaled_expression <- t(scale(t(smoothed_expression)))
    ...
    gene_clusters <- stats::cutree(gene_hclust, k = min(n_clusters, length(top_genes)))
    ...
    top_genes <- top_genes[cluster_order]
    smoothed_expression <- smoothed_expression[top_genes, ]
  }
  ...
  if (plot_type %in% c("heatmap", "both")) {
    ...
    if (requireNamespace("ComplexHeatmap", quietly = TRUE) &&
      ...
      if (cluster_genes) {
        ...
        cluster_colors <- circlize::colorRamp2(
          seq(1, n_clusters),
          rainbow(n_clusters)
        )
      ...
      col_fun <- circlize::colorRamp2(
        c(min(heatmap_data),
          (min(heatmap_data) + max(heatmap_data))/2,
          max(heatmap_data)),
        c("blue", "white", "red")
      )
      ...
      hm <- ComplexHeatmap::Heatmap(
        ...
        cluster_rows = FALSE,  # Already clustered if requested
  ...
  if (plot_type %in% c("line", "both")) {
    ...
    if (requireNamespace("ggplot2", quietly = TRUE) &&
      ...
      if (cluster_genes) {
        ...
        for (cluster in unique(line_data$Cluster)) {
          cluster_data <- line_data[line_data$Cluster == cluster, ]

          p <- ggplot2::ggplot(cluster_data, ggplot2::aes(x = Pseudotime, y = Expression, color = Gene, group = Gene)) +
            ggplot2::geom_line() +
            ggplot2::theme_minimal() +
        ...
        p_facet <- ggplot2::ggplot(line_data, ggplot2::aes(x = Pseudotime, y = Expression, color = Gene, group = Gene)) +
          ggplot2::geom_line() +
        ...
        p <- ggplot2::ggplot(line_data, ggplot2::aes(x = Pseudotime, y = Expression, color = Gene, group = Gene)) +
          ggplot2::geom_line() +
          ggplot2::theme_minimal() +
...
diffTrajectoryAnalysis <- function(sce,
  ...
  for (group in groups) {
    group_bin_expression[[group]] <- matrix(0, nrow = nrow(expression_matrix), ncol = n_bins)
